...
fn outputs_tostring(instr_index: usize, inputs: &Vec<OutputSlot>) -> String {
    inputs.iter().enumerate().map(|(output_index, slot)| {
        if output_index == 0 {
...
fn inputs_tostring(inputs: &Vec<InputSlot>) -> String {
    inputs.iter().map(|slot| {
        match slot {
            ...
            InputSlot::InstructionOutput { instruction_index, tp: _, output_index } => {
                if *output_index == 0 {
...
pub fn block_tostring(block: &IRBlock) -> String {
    return block.instructions.iter().map(|instruction| {
