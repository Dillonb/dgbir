...
fn write_ptr() {
    ...
    func.write_ptr(&block, DataType::U32, const_ptr(&r as *const u32 as usize), const_u32(1));
...
fn add_write_ptr() {
    ...
    func.write_ptr(&block, DataType::U32, const_ptr(&r as *const u32 as usize), add3_result.val());
    func.ret(&block, None);
    println!("{}", func);
    interpret_func(&func);
    assert_eq!(r, 5);
...
fn write_float_ptr() {
    ...
    func.write_ptr(&block, DataType::F32, const_ptr(&r as *const f32 as usize), const_f32(1.0));
    func.ret(&block, None);
...
fn add_write_float_ptr() {
    ...
    func.write_ptr(&block, DataType::F32, const_ptr(&res_1 as *const f32 as usize), add_result.val());

    func.write_ptr(&block, DataType::U32, const_ptr(&res_2 as *const u32 as usize), add_result_2.val());
    func.ret(&block, None);
